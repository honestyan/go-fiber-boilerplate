...
func (authorL) LoadArticles(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAuthor interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`article`),
		qm.WhereIn(`article.author_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(authorPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert author, could not build conflict column list")
			}

			conflict = make([]string, len(authorPrimaryKeyColumns))
			copy(conflict, authorPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"author\"", updateOnConflict, ret, update, conflict, insert, opts...)
